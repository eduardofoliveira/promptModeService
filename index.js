        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                if (item.did === "tronco") {
                    ...
                    await log.addLog(
                        `${item.did} alterado para ${item.destino} no dominio ${item.dominio} com retorno: ${data.message}`
                    )
                } else {
                    let { data } = await axios.get(`${url}/set`, {
                        params: {
...
let job = new CronJob(
    ...
    async () => {
        // agora = new Date();
        agora = new Date(2021, 05, 17, 18, 0, 0)
        let diaSemana = agora.getDay()
        let hora = agora.getHours()
        let minuto = agora.getMinutes()
                ...
                try {
                    if (item.did === "tronco") {
                        let response = await axios.get(
                            `${url[0].valor}/setdefaultoperator/${item.destino}&${item.dominio}`
                        )
                        ...
                        if (verif.test(response.data)) {
                            ...
                            await conn.query("INSERT INTO log (informacao) values (?)", [
                                `Alteração efetuada: ${response.data}`
                            ])
                        }
                    } else {
                        let response = await axios.get(
                            `${url[0].valor}/set/${item.destino}&${item.dominio}&${item.did}`
                        )
